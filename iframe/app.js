...
(function() {
  ...
  function normalizePrefs(input) {
    ...
    return {
      lang: (raw == null ? void 0 : raw.lang) === "en" ? "en" : "zh",
      theme: (raw == null ? void 0 : raw.theme) === "dark" ? "dark" : "light",
      features
    };
  }
  ...
  function reportTitle(kind, snapshot, extra) {
    ...
    if (kind === "context") return t(`\u5DE5\u7A0B\u5FEB\u7167\u8BB0\u5F55 \xB7 ${projectLabel}`, `Snapshot Record \xB7 ${projectLabel}`);
...
`], { type: "application/json;charset=utf-8" }), `${safeFilePart(report.title, "design-pulse-report")}.json`);
  ...
  async function generateContextReport() {
    ...
    setStatus("success", t("\u5DF2\u8BB0\u5F55\u5F53\u524D\u5DE5\u7A0B\u5FEB\u7167\u3002", "Current design snapshot recorded."));
    render();
  }
  ...
  async function clearReportHistory() {
    if (!window.confirm(t("\u786E\u8BA4\u6E05\u7A7A\u5168\u90E8\u8BB0\u5F55\u5386\u53F2\uFF1F", "Clear all history?"))) return;
    state.reportHistory = [];
    await saveReportHistory();
    setStatus("success", t("\u8BB0\u5F55\u5386\u53F2\u5DF2\u6E05\u7A7A\u3002", "History cleared."));
    render();
  }
  function typeMap() {
  ...
  function header() {
    const actionButtons = [
      ...
      featureEnabled("enableReportsView") ? `<button class="ghost-button" data-action="view" data-view="reports">${e(t("\u5DE5\u7A0B\u8BB0\u5F55", "Logs"))}</button>` : "",
    ...
    return `<header class="app-header"><div><p class="eyebrow">JLCEDA Plugin</p><h1>${e(t("\u7269\u6599\u7BA1\u7406\u52A9\u624B", "Material Manager"))}</h1><p class="hero-copy">${e(t("\u56F4\u7ED5\u7269\u6599\u7BA1\u7406\u3001\u5F53\u524D\u5DE5\u7A0B\u4E0A\u4E0B\u6587\u3001\u5236\u9020\u5BFC\u51FA\u3001\u753B\u5E03\u622A\u56FE\u4E0E BOM \u534F\u4F5C\uFF0C\u7EDF\u4E00\u5904\u7406\u5609\u7ACB\u521B EDA \u5E38\u7528\u5DE5\u7A0B\u52A8\u4F5C\u3002", "A focused JLCEDA Pro tool for material management, design context, manufacture exports, canvas capture, and BOM workflows."))}</p></div><div class="header-actions button-cluster button-cluster-tight">${actionButtons}</div></header>`;
  }
  function nav() {
    const items = [
      ["dashboard", "\u6982\u89C8", "Overview"],
      featureEnabled("enableReportsView") ? ["reports", "\u8BB0\u5F55", "Logs"] : null,
  ...
  function currentEdaSnapshotCard() {
    ...
    const buttons = [
      ...
      featureEnabled("enableReportsView") ? `<button class="ghost-button" type="button" data-action="generate-context-report">${e(t("\u8BB0\u5F55\u5FEB\u7167", "Record Snapshot"))}</button>` : "",
      featureEnabled("enableReportsView") ? `<button class="ghost-button" type="button" data-action="view" data-view="reports">${e(t("\u67E5\u770B\u8BB0\u5F55", "Open Logs"))}</button>` : "",
    ...
    return `<article class="panel-card"><div class="section-head"><h2>${e(t("\u5F53\u524D\u5DE5\u7A0B\u5FEB\u7167", "Current Design Snapshot"))}</h2><div class="inline-actions button-cluster">${buttons}</div></div><p class="support-text">${e(subtitle)}</p>${details}</article>`;
  }
  function render() {
    normalizeActiveView();
  ...
  function dashboardView() {
    ...
    const quickActions = [
      featureEnabled("enableReportsView") ? `<button class="primary-button" data-action="generate-context-report">${e(t("\u8BB0\u5F55\u5DE5\u7A0B\u5FEB\u7167", "Record Snapshot"))}</button>` : `<button class="primary-button" data-action="view" data-view="components">${e(t("\u65B0\u589E\u5143\u5668\u4EF6", "Add Component"))}</button>`,
      ...
      featureEnabled("enableReportsView") ? `<button class="ghost-button" data-action="view" data-view="reports">${e(t("\u67E5\u770B\u8BB0\u5F55\u5386\u53F2", "Open History"))}</button>` : "",
    ...
    return `<section class="card-grid summary-grid"><article class="summary-card accent-blue"><span>${e(t("\u5143\u5668\u4EF6", "Components"))}</span><strong>${state.db.components.length}</strong></article><article class="summary-card accent-gold"><span>${e(t("\u5E93\u5B58\u9884\u8B66", "Warnings"))}</span><strong>${warningCount}</strong></article><article class="summary-card accent-green"><span>${e(t("\u8BB0\u5F55\u5386\u53F2", "History"))}</span><strong>${reportCount}</strong></article><article class="summary-card accent-red"><span>${e(t("BOM \u660E\u7EC6", "BOM Items"))}</span><strong>${bomCount}</strong></article></section><section class="card-grid two-col"><article class="panel-card"><h2>${e(t("\u5FEB\u901F\u5165\u53E3", "Quick Actions"))}</h2><div class="quick-actions button-cluster">${quickActions}</div><div class="quick-actions button-cluster button-cluster-tight">${featureEnabled("enableExportHub") ? `<button class="ghost-button" data-action="export-manufacture" data-kind="bom">${e(t("\u5FEB\u901F\u5BFC\u51FA BOM", "Quick Export BOM"))}</button>` : ""}${featureEnabled("enableExportHub") ? `<button class="ghost-button" data-action="export-manufacture" data-kind="gerber">${e(t("\u5FEB\u901F\u5BFC\u51FA Gerber", "Quick Export Gerber"))}</button>` : ""}${featureEnabled("enableCanvasTools") ? `<button class="ghost-button" data-action="capture-canvas-current">${e(t("\u622A\u56FE\u5F53\u524D\u89C6\u56FE", "Capture Current View"))}</button>` : ""}${featureEnabled("enableCanvasTools") ? `<button class="ghost-button" data-action="capture-canvas-selected">${e(t("\u622A\u56FE\u5F53\u524D\u9009\u533A", "Capture Selection"))}</button>` : ""}${featureEnabled("enableCanvasTools") ? `<button class="ghost-button" data-action="mark-canvas-selected">${e(t("\u6846\u9009\u5F53\u524D\u9009\u533A", "Mark Selection"))}</button>` : ""}</div></article>${snapshotCard}</section><section class="panel-card"><h2>${e(t("\u64CD\u4F5C\u8981\u70B9", "Workflow Notes"))}</h2><ul class="info-list"><li>${e(t("\u201C\u8BB0\u5F55\u201D\u9875\u7528\u4E8E\u4FDD\u5B58\u5F53\u524D\u753B\u5E03\u3001\u56E2\u961F\u548C\u5DE5\u4F5C\u533A\u4E0A\u4E0B\u6587\uFF0C\u4FBF\u4E8E\u540E\u7EED\u56DE\u770B\u3002", "Use Logs to keep the current design, team, and workspace context for later review."))}</li><li>${e(t("\u201C\u5BFC\u51FA\u4E2D\u5FC3\u201D\u96C6\u4E2D\u5904\u7406 Gerber\u3001\u5750\u6807\u30013D\u3001\u7F51\u8868\u548C\u6D4B\u8BD5\u70B9\u8D44\u6599\u5BFC\u51FA\u3002", "Use Export Hub to keep Gerber, Pick&Place, 3D, netlist, and test point exports in one place."))}</li><li>${e(t("\u7A97\u53E3\u6807\u9898\u680F\u652F\u6301\u6700\u5927\u5316\u4E0E\u6700\u5C0F\u5316\uFF1B\u7A97\u53E3\u6700\u5C0F\u5316\u540E\uFF0C\u518D\u6B21\u70B9\u51FB\u63D2\u4EF6\u83DC\u5355\u5373\u53EF\u6062\u590D\u3002", "The iframe window supports maximize and minimize. Click the plugin menu again to restore a minimized window."))}</li><li>${e(t("\u5BFC\u5165\u5F53\u524D\u5DE5\u7A0B BOM \u524D\uFF0C\u53EF\u5148\u8BFB\u53D6\u5DE5\u7A0B\u5FEB\u7167\u786E\u8BA4\u5199\u5165\u6765\u6E90\u3002", "Load the design snapshot first if you want to confirm the project and PCB source before importing the current BOM."))}</li></ul></section>`;
  }
  function reportsView() {
    const snapshot = state.edaSnapshot;
    const contextPanel = `<section class="panel-card"><div class="section-head"><h2>${e(t("\u5DE5\u7A0B\u8BB0\u5F55", "Design Logs"))}</h2><div class="inline-actions button-cluster button-cluster-tight"><button class="primary-button" type="button" data-action="generate-context-report">${e(t("\u8BB0\u5F55\u5F53\u524D\u5FEB\u7167", "Record Snapshot"))}</button><button class="ghost-button" type="button" data-action="refresh-eda-snapshot">${e(t("\u5237\u65B0\u4E0A\u4E0B\u6587", "Refresh Context"))}</button></div></div><p class="support-text">${e(snapshot ? t(`\u5F53\u524D\u6587\u6863\uFF1A${snapshot.currentDocumentType || "-"}\uFF0C\u56E2\u961F\uFF1A${snapshot.teamName || "-"}\uFF0C\u5DE5\u4F5C\u533A\uFF1A${snapshot.workspaceName || "-"}`, `Current document: ${snapshot.currentDocumentType || "-"}, team: ${snapshot.teamName || "-"}, workspace: ${snapshot.workspaceName || "-"}`) : t("\u5148\u8BFB\u53D6\u5F53\u524D\u5DE5\u7A0B\u5FEB\u7167\uFF0C\u518D\u5199\u5165\u8BB0\u5F55\u3002", "Load the current design snapshot before writing a log entry."))}</p><div class="meta-grid"><div><span>${e(t("\u5DE5\u7A0B", "Project"))}</span><strong>${e(projectDisplayNameFromSnapshot(snapshot) || "-")}</strong></div><div><span>${e(t("PCB", "PCB"))}</span><strong>${e(pcbDisplayNameFromSnapshot(snapshot) || "-")}</strong></div><div><span>${e(t("\u677F\u5B50", "Board"))}</span><strong>${e(boardDisplayNameFromSnapshot(snapshot) || "-")}</strong></div><div><span>${e(t("\u6587\u6863\u7C7B\u578B", "Document Type"))}</span><strong>${e((snapshot == null ? void 0 : snapshot.currentDocumentType) || "-")}</strong></div><div><span>${e(t("\u6587\u6863 UUID", "Document UUID"))}</span><strong>${e((snapshot == null ? void 0 : snapshot.currentDocumentUuid) || "-")}</strong></div><div><span>${e(t("\u6807\u7B7E\u9875 ID", "Tab ID"))}</span><strong>${e((snapshot == null ? void 0 : snapshot.currentTabId) || "-")}</strong></div><div><span>${e(t("\u56E2\u961F", "Team"))}</span><strong>${e((snapshot == null ? void 0 : snapshot.teamName) || "-")}</strong></div><div><span>${e(t("\u56E2\u961F\u8EAB\u4EFD", "Team Identity"))}</span><strong>${e((snapshot == null ? void 0 : snapshot.teamIdentity) !== "" ? snapshot == null ? void 0 : snapshot.teamIdentity : "-")}</strong></div><div><span>${e(t("\u5DE5\u4F5C\u533A", "Workspace"))}</span><strong>${e((snapshot == null ? void 0 : snapshot.workspaceName) || "-")}</strong></div><div><span>${e(t("\u5DE5\u4F5C\u533A UUID", "Workspace UUID"))}</span><strong>${e((snapshot == null ? void 0 : snapshot.workspaceUuid) || "-")}</strong></div><div><span>${e(t("\u9879\u76EE UUID", "Project UUID"))}</span><strong>${e((snapshot == null ? void 0 : snapshot.currentDocumentProjectUuid) || (snapshot == null ? void 0 : snapshot.projectUuid) || "-")}</strong></div></div></section>`;
    const canvasButtons = featureEnabled("enableCanvasTools") ? `<section class="panel-card"><div class="section-head"><h2>${e(t("\u753B\u5E03\u5DE5\u5177", "Canvas Tools"))}</h2><div class="inline-actions"><button class="ghost-button" type="button" data-action="clear-canvas-markers">${e(t("\u6E05\u9664\u6807\u8BB0", "Clear Markers"))}</button></div></div><div class="quick-actions button-cluster"><button class="ghost-button" type="button" data-action="zoom-canvas-all">${e(t("\u9002\u5E94\u5168\u90E8", "Fit All"))}</button><button class="ghost-button" type="button" data-action="zoom-canvas-selected">${e(t("\u9002\u5E94\u9009\u4E2D", "Fit Selected"))}</button><button class="ghost-button" type="button" data-action="mark-canvas-all">${e(t("\u6846\u9009\u5168\u90E8\u8303\u56F4", "Mark All"))}</button><button class="ghost-button" type="button" data-action="mark-canvas-selected">${e(t("\u6846\u9009\u5F53\u524D\u9009\u533A", "Mark Selected"))}</button><button class="primary-button" type="button" data-action="capture-canvas-current">${e(t("\u622A\u56FE\u5F53\u524D\u89C6\u56FE", "Capture Current View"))}</button><button class="primary-button" type="button" data-action="capture-canvas-all">${e(t("\u622A\u56FE\u5168\u90E8\u56FE\u5143", "Capture All"))}</button><button class="primary-button" type="button" data-action="capture-canvas-selected">${e(t("\u622A\u56FE\u5F53\u524D\u9009\u533A", "Capture Selection"))}</button></div><p class="support-text">${e(t("\u9002\u5408\u5728 PCB/\u539F\u7406\u56FE\u6392\u67E5\u65F6\u5FEB\u901F\u6846\u9009\u3001\u622A\u56FE\u5E76\u5199\u5165\u8BB0\u5F55\u5386\u53F2\u3002", "Use these tools to frame, capture, and record canvas context while reviewing PCB or schematic documents."))}</p></section>` : "";
    const historyItems = state.reportHistory.map((item) => {
      var _a2;
      const extraPills = [
...
`;
  ...
  if (featureEnabled("autoLoadEdaSnapshot") && hasAnyEdaEntryEnabled()) {
    void refreshCurrentEdaSnapshot({ silent: true }).catch(() => void 0);
  }
})();
