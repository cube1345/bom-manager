...
1. 收敛首页、记录页与导出页的文案，去掉偏助手/提示词表达
1. 整理头部、导航、卡片、按钮和表格样式层级，提升桌面与窄屏可读性
1. 补齐状态条、嵌套列表和移动端按钮布局的一致性
# 0.1.10

## 变更

1. 关于对话框补充插件核心能力概览
1. 让安装后首次打开时更容易理解工程脉搏的定位
1. 作为本轮 20 次迭代的收束版本完成 0.1.10 发布
# 0.1.9

## 变更

1. 环境自检说明补充画布截图/框选与制造导出能力的接口解读
1. 让自检结果更贴合工程脉搏当前的功能边界
1. 出现能力缺失时更容易判断是画布 API 还是制造 API 不可用
# 0.1.8

## 变更

1. 报告历史卡片新增截图区域坐标显示
1. 让回看截图记录时可以判断当时聚焦的画布范围
1. 提升截图类历史记录的可解释性
# 0.1.7

## 变更

1. 补充扩展关键词为 EDA/Manufacturing/BOM/Snapshot
1. 让插件定位更贴近工程快照与制造协同场景
1. 为后续分发和检索补齐更明确的元数据
# 0.1.6
